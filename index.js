...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.gun4g.mongodb.net/myFirstDatabase?retryWrites=true&w=majority`;
...
async function run() {
    ...
    app.get('/booking/:id',verifyJWT,async(req,res)=>{
      ...
      app.post('/booking', async(req,res)=>{
        ...
        console.log(booking);
        const query={treatment:booking.treatment ,date:booking.date, patient:booking.patient, price : booking.price}
        const exist = await bookingCollection.findOne(query);
        if(exist){
          return  res.send({succes:false, booking:exist})
        }
...
app.listen(port, () => {
  console.log(`Doctor's poratl listening on port ${port}`)
})